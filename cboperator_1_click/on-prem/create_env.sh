...
#read options
    case $options in
		...
		echo "Creating Nifi POD"
	    	kubectl -n ${ns} create -f templates/nifi.yaml
	    	sleep 15
		...
		bash ./scripts/cb_no_ldap.sh ${ns} ${cluster}
            ;;
        5)
		bash demos/mysql-kafka-cb/create_env.sh
	    ;;
	6) 
		bash demos/couchmovies/couchmovies.sh
	   ;;
	7)
                bash scripts/couchmart_no_ldap.sh
           ;;
	8)
		bash demos/stock-ex/create_env.sh
	  ;;
	9)
		bash demos/travel-sample/create_env.sh
	  ;;
        10)
            break
            ;;
