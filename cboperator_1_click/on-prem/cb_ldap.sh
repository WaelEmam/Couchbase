...
kubectl run openldap --image=osixia/openldap:1.3.0 --port=389 --env LDAP_ORGANISATION="Couchbase" --env LDAP_DOMAIN="${ldap_domain}.couchbase.com" --env LDAP_ADMIN_PASSWORD="${ldap_pass}"
...
ldapadd -x -H ldap://localhost:389 -D "cn=admin,dc=${ldap_domain},dc=couchbase,dc=com" -w ${ldap_pass} -f ldap_test
